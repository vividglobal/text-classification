...
from text_recognition.utils import CTCLabelConverter, CTCLabelConverterForBaiduWarpctc, AttnLabelConverter, Averager
from text_recognition.dataset import hierarchical_dataset, AlignCollate, Batch_Balanced_Dataset
from text_recognition.model import Model
from text_recognition.test import validation
...
def train(opt):
    ...
    if opt.saved_model != '':
        ...
        if opt.FT:
            ...
            state_dict = {k: v for k, v in torch.load(opt.saved_model).items() if k not in ignored_keys}
